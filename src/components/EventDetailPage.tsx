...
export const EventDetailPage: React.FC = () => {
  ...
  if (notFound || !event) {
    return (
      <div className="flex flex-col h-screen items-center justify-center bg-background px-4">
        <SEOHead 
          title="Event Not Found - TechArena"
        ...
        <div className="text-center mt-20">
          ...
          <p className="text-lg text-muted-foreground mb-8">
            The competition or event you're looking for doesn't exist or has been removed.
          </p>
          <button
            onClick={() => navigate('/discover')}
            className="px-6 py-3 bg-primary text-primary-foreground border border-primary hover:bg-primary/90 transition-colors uppercase text-sm font-medium"
          >
            Browse Events
          </button>
        </div>
  ...
  return (
    <>
      <SEOHead 
        title={`${event.title} - TechArena`}
        description={event.description.substring(0, 160)}
        image={event.background_image_url}
        keywords={`hackathon, competition, ${event.title}, ${event.address}, tech event`}
      />
      <Navbar />
